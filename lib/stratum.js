...
var StratumClient = function(options){
    ...
    function handleAuthorize(message, replyToSocket){
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function(result) {
            ...
            else // check password for fixed diff
            {
                if(_this.workerPass.substring(0,2) == "d=")
                {
                    var diff = parseFloat(_this.workerPass.substring(2,10));
                    if(Number.isFinite(diff))
                    {
                        //range check
                        if(diff >= 0.1 && diff <= 8192)
                        {
                            _this.fixedDiff = true;
                            _this.enqueueNextDifficulty(diff);
                        }
                    }
                }
            }
