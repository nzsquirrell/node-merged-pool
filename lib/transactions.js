...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData, coin){
    ...
    else if (coin.symbol == 'NOIR')
    {
        if(rpcData.noirnode_payments_started)
        {
            if (rpcData.noirnode.payee && rpcData.noirnode.amount)
            {
                var payeeScript = util.addressToScript(rpcData.noirnode.payee);
                txOutputBuffers.push(Buffer.concat([
                    util.packInt64LE(rpcData.noirnode.amount),
                    util.varIntBuffer(payeeScript.length),
                    payeeScript
                ]));
            }
        }
        if (rpcData.height > 435005)
        {
            var payeeScript = util.addressToScript("ZL2juii5Y6z9Fnsd9Y1dRRvFC33CR9aDj8");
            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(10000000),
                util.varIntBuffer(payeeScript.length),
                payeeScript
            ]));
            var payeeScript = util.addressToScript("ZU3KK4pYsE5sqJo9zDwyoup1wpzUe5HT9H");
            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(10000000),
                util.varIntBuffer(payeeScript.length),
                payeeScript
    ...
    if(rpcData.founder_payments_started)
    {
        if(rpcData.founder)
        {
            ...
            var founderScript = util.scriptAddressToScript(rpcData.founder.payee);
            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(founderReward),
                util.varIntBuffer(founderScript.length),
                founderScript
            ]));
        }
    }
    ...
    if(coin.symbol == 'HXX')
    {
        ...
        var founderScript3 = util.addressToScript('HG1utYiVhkgBNz5ezrVpsjABxmMdVdcQe5');
