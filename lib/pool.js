...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    var emitLog        = function(text) { _this.emit('log', 'debug'  , options.coin, text); };
    var emitInfoLog    = function(text) { _this.emit('log', 'info'   , options.coin, text); };
    var emitWarningLog = function(text) { _this.emit('log', 'warning', options.coin, text); };
    var emitErrorLog   = function(text) { _this.emit('log', 'error'  , options.coin, text); };
    var emitSpecialLog = function(text) { _this.emit('log', 'special', options.coin, text); };
    ...
    function OutputPoolInfo(){
        ...
        var infoLines = [startMessage,
                ...
                'Network Hash Rate:\t' + util.getReadableHashRateString(Math.pow(2, 32) * options.initStats.difficulty / algos[options.coin.algorithm].multiplier / (options.coin.blocktime ? options.coin.blocktime : 150)),
    ...
    function OnBlockchainSynced(syncedCallback){
        var checkSynced = function(displayNotSynced){
            ...
            _this.daemon.cmd('getblocktemplate', gbtParams, function(results){
        ...
        var generateProgress = function(){
            ...
            _this.daemon.cmd((options.coin.legacy ? 'getinfo' : 'getblockchaininfo'), [], function(results) {
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            var emitShare = function(){
                _this.emit('share', options.coin, options.auxes, isValidShare, isValidBlock, shareData);
            };
            ...
            */
            ...
            _this.emit('log', severity, options.coin, message);
        });
    }
    ...
    function DetectCoinData(finishedCallback){
        ...
        var batchRpcCalls = [
            ...
            ['getdifficulty', []],
            [(options.coin.legacy ? 'getinfo' : 'getnetworkinfo'), []],
            ['getmininginfo', []],
            ['submitblock', []]
        ];
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            options.poolAddressScript = (function(){
                switch(options.coin.reward){
                    ...
                    case 'POW':
                        if (options.coin.symbol === "NIX")
                            return util.addressToP2sh(rpcResults.validateaddress.address, options.coin.network);
                        else
                            return util.addressToScript(rpcResults.validateaddress.address);
                }
            })();

            options.testnet = options.coin.legacy ? rpcResults.getinfo.chain : rpcResults.getnetworkinfo.chain;
            options.protocolVersion = options.coin.legacy ? rpcResults.getinfo.protocolversion : rpcResults.getnetworkinfo.protocolversion;

            options.initStats = {
                connections: options.coin.legacy ? rpcResults.getinfo.connections : rpcResults.getnetworkinfo.connections,
